...
![image-20220618154724921](https://s2.loli.net/2022/06/18/mOLrwSE1FehH9Bl.png)
...
![image-20220620200502624](https://s2.loli.net/2022/06/20/Aq2edPJMrKkYcEa.png)
...
![image-20220621161009471](https://s2.loli.net/2022/06/21/1HXdguPrk9D86lW.png)
...
![image-20220621163000754](https://s2.loli.net/2022/06/21/faozd9IkSWmgcle.png)



#### 交替编号法

![image-20220621163646494](https://s2.loli.net/2022/06/21/bmJA7ruHdUwc65t.png)
...
![image-20220621163521936](https://s2.loli.net/2022/06/21/hrfY9s5d4ApZCBz.png)

![image-20220621163635896](https://s2.loli.net/2022/06/21/QR7dTjuoZMfaCiA.png)具体做法：让相邻盘面的扇区编号“错位”，可以在第一次划过1号盘面后，就可以直接读取数据数据，减少了延迟时间
...
Step 1:**进行低级格式化（物理格式化）**，将磁盘的各个磁道划分为扇区。一个扇区通常可分为头、数据区域（如512B大小）、尾三个部组成。管理扇区所需要的各种数据结构一般存放在头、尾两个部分，包括扇区校验码（如奇偶校验、CRC 循环冗余校验码等，校验码用于校验扇区中的数据是否发生错误）
...
开机时计算机先运行“自举装入程序”，通过执行该程序就可找到引导块，开将元整的“目车程序”读入内存，完成初始化
...
​	![image-20220624152436340](https://s2.loli.net/2022/06/24/t26dY38xGzB5IfM.png)
...
如我们看得见摸得看鼠/提盘按钮；显示器的LED屏；移动硬盘的磁臂、磁盘盘面。
...
- **向CPU报告设备的状态**：**状态寄存器**，记录当前设备状态
- **数据交换**：**数据寄存器**
- **地址识别**：判断CPU要操作的是哪个寄存器

![image-20220624155015658](https://s2.loli.net/2022/06/24/uyRgo3JerGp4NT7.png)
...
![image-20220624155124503](https://s2.loli.net/2022/06/24/jhfDck8QGup1ALE.png)
...
![image-20220624155256843](https://s2.loli.net/2022/06/24/y9AMW3UiZSVj8lY.png)
...
![image-20220625100516423](C:\Users\AiYou_\AppData\Roaming\Typora\typora-user-images\image-20220625100516423.png)

![image-20220625100747101](C:\Users\AiYou_\AppData\Roaming\Typora\typora-user-images\image-20220625100747101.png)
...
缺点：CPU和I/O只能串行工作，CPU需要一直轮训检查，长期处于忙等的状态
...
![image-20220625101610008](C:\Users\AiYou_\AppData\Roaming\Typora\typora-user-images\image-20220625101610008.png)

​	引入**中断机制**。由于I/O设备速度很慢，因此在CPU发出读/写命令后，可将**等待I/O的进程阻塞**，先切换到别的进程执行。当I/O完成后，控制器会向CPU发出一个中断信号，CPU检**测到中断信号后**，会保存当前进程的运行环境信息，转去执行中断处理程处理该中断。处理中断的过程中，CPU从I/O控制器读一个字的数据传送到CPU寄存器，再写入主存。接着，CPU恢复等待I/O的进程（或其他进程）的运行环境，然后继续执行。
...
​	与“中断驱动方式”相比，DMA方式(Direct Memory Access,直接存储器存取。主要用于块设备的/O控制)有这样几个改进：

​	①**数据的传送单位是“块”**。不再是一个字、一个字的传送；

​	②数据的流向是**从设备直接放入内存、或者从内存直接到设备**。不再需要CPU作为“快递小哥”。

​	③仅在传送一个或多个数据块的开始和结束时，才需要CPU于预。

![image-20220625102602217](C:\Users\AiYou_\AppData\Roaming\Typora\typora-user-images\image-20220625102602217.png)

![image-20220625103018865](C:\Users\AiYou_\AppData\Roaming\Typora\typora-user-images\image-20220625103018865.png)
...
优点：传输以块为单位，CPU接入频率进一步降低

缺点：CPU发出IO命令只能读写一个块或者连续的数据块；离散块需要多次IO
...
![image-20220625145834737](https://s2.loli.net/2022/06/25/DJm3UjYT5NieA9H.png)
...
![image-20220625150045690](https://s2.loli.net/2022/06/25/hqtuxLRjMQSgTyY.png)





![image-20220625150124843](https://s2.loli.net/2022/06/25/zyvcoD862xga3PU.png)
...
![image-20220625151900863](https://s2.loli.net/2022/06/25/hio64CsYrF1cxKT.png)



设备独立性软件需要通过逻辑设备表来确定逻辑设备对应的物理设备，并找到该设备对应的设备驱动程序。

![image-20220625152023747](https://s2.loli.net/2022/06/25/qPvTBh2upmOEkct.png)

用户通过调用用户层软件提供的库函数发出的I/0请求
...
→等待1/0完成的进程应该被阻塞,因此需要进程切换,而进程切换必然需要中断处理,
...
![image-20220625152653406](https://s2.loli.net/2022/06/25/chQ5IyK3mSEMidt.png)
...
![image-20220625153630514](https://s2.loli.net/2022/06/25/kfDSMCgsb2AFmwP.png)

“假脱机技术”又称为“spooling”技术是用软件的方式模拟脱机技术

![image-20220625153341746](https://s2.loli.net/2022/06/25/QB6jzHJFCPKkUaI.png)

“输入井”模拟脱机输入时的磁带，用于收容/O 收备输入的数据
...
在输入进程的控制下，“输入缓冲区”用王斯左从输反备输入的数据，之后再转存到输入井中

在输出进程的控制下，“输出缓冲区”用于暂存从输出井送来的数据，之后再传送到输出设备上



独占式设备一**只允许各个进程串行使用的设备**

共享设备一一**允许多个进程“同时”使用的设备**

![image-20220625153945335](https://s2.loli.net/2022/06/25/l9yCFsP4tDKYXvu.png)
...
​	SPOOLing技术可以把一台物理设备虚拟成逻辑上的多台设备， 可将独占式设备改造成井享设备。
...
共享设备一-可同时分配给多个进程使用(如磁盘),各进程往往是宏观上同时共享使用设备,而微观上交替使用。

虚拟设备--采用SPOOLing技术将独占设备改造成虚拟的共享设备,可同时分配给多个进程使用(如采用SPOOLing技术实现的共享打印机)

**设备分配算法**：先来先服务，优先级高者优先，短任务优先等等

**设备分配安全性**：

安全分配方式：为进程分配一个设备后就将进程组塞，本次I/0完成后才将进程唤醒

不安全分配方式：进程发出I/O请求后。系统为其分配I/o设备，进程可继续执行，之后还可以发出新的I/0请求。只有某个I/0请求得不到满足时才将进程阻基。
...
![image-20220625155925984](https://s2.loli.net/2022/06/25/QgksVqmWnDUurah.png)

**设备控制表(DCT):**系统为每个设备配置一张DCT,用于记录设备情况

![image-20220625155904465](https://s2.loli.net/2022/06/25/fWdvz5y12rYa98D.png)**控制器控制表(COCT):**每个设备控制器都会对应一张COCT。操作系统根据COCT的信息对控制器进行深作和管理。

![image-20220625155938768](https://s2.loli.net/2022/06/25/E4O67qpZLKDvR8n.png)

**通道控制表（CHCT)**:每个通道都会对应一张CHCT。操作系统根据CHCT的信息对通道进行操作和管理。

![image-20220625160034952](https://s2.loli.net/2022/06/25/JtPgVyLAc7bvsZD.png)**系统设备表（SDT)**：记录了系统中全部设备的情况，每个设备对应一个表目。

![image-20220625160125482](https://s2.loli.net/2022/06/25/umU1SVT7iXArhYw.png)
...
③根据DCT找到COCT,若控制器忙碌则将进程PCB挂到控制器等待队列中，不忙碌则将控制器分配给讲程。
...
②查找SDT,找到用户进程指定类刑的、并日空闲的设备,将其分配给该进程。操作系统在逻辑设备表(LUT)中新增一个表项。

③根据DCT找到cocT,若控制器忙碌则将进程PCB挂到控制器等待队列中,不忙碌则将控制器分配给进程。

④根据COCT找到CHCT,若通道忙碌则将进程PCB挂到通道等待队列中,不忙碌则将通道分配给进程

![image-20220625160506405](https://s2.loli.net/2022/06/25/8tETuLiO6K7lYbM.png)
